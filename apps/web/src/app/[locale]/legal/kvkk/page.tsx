  ...
  if (locale === 'en') {
    return (
      <article>
        ...
        <p className={P}>
          KVKK is the Turkish Personal Data Protection Law (Law No. 6698) governing how YapıOps
          processes personal data of users in Türkiye. The full, legally binding text of this notice
  ...
  return (
    <article>
      ...
      <p className={P}>
        ...
        <strong>BlueDev</strong>&apos;tir (YapıOps platformu).
      </p>
      <p className={P}>
        İletişim:{' '}
      ...
      <ul className={UL}>
        <li>
          <strong>Kimlik:</strong> Ad, soyad, TCKN (Ek-3 yapı sahibi alanı), VKN (kurumsal
          müteahhit/denetim firmaları)
        </li>
        <li>
          <strong>İletişim:</strong> E-posta adresi, telefon
        </li>
        <li>
          <strong>Mesleki:</strong> İMO oda sicil numarası, sorumlu mühendis bilgisi, müteahhit/yapı
          denetim firma kayıtları
        </li>
        <li>
          <strong>İşlem güvenliği:</strong> IP adresi, kullanıcı aracı (user agent), oturum
          bilgileri, audit log kayıtları
        </li>
        <li>
          <strong>Müşteri işlem:</strong> Hesap kullanım istatistikleri, üretilen Ek-3 sayısı,
          abonelik durumu
        </li>
        <li>
          <strong>Finansal:</strong> Fatura adresi, ödeme tutarı (kart bilgileri{' '}
          <strong>Iyzico</strong> aracılığıyla işlenir; YapıOps kart bilgisi saklamaz)
        </li>
      </ul>
      ...
      <ul className={UL}>
        <li>
          4708 sayılı Yapı Denetimi Hakkında Kanun gereği Ek-3 (Yapı Denetim Hizmet Sözleşmesi)
          elektronik formunun üretilmesi ve arşivlenmesi
        </li>
        <li>Hesap güvenliği, kimlik doğrulama (multi-tenant org yapısı)</li>
        <li>Ödeme tahsilatı ve e-fatura kesimi (213 sayılı VUK, 6502 sayılı Tüketici Kanunu)</li>
      ...
      <ul className={UL}>
        ...
        <li>KVKK Madde 5/2(f) — Meşru menfaat (güvenlik, dolandırıcılık önleme)</li>
      </ul>

      <h2 className={H2}>5. Verilerin Aktarımı</h2>
      <p className={P}>
        Hizmet altyapısı için aşağıdaki sağlayıcılar veri işleyici sıfatıyla görev alır:
      </p>
      ...
      <p className={P}>
        Yurt dışı aktarımlar KVKK Madde 9 kapsamında açık rıza ile yapılır. Ayrıca yetkili kamu
        kurum ve kuruluşlarına (4708 sayılı Kanun çerçevesinde Çevre, Şehircilik ve İklim
        Değişikliği Bakanlığı), yasal zorunluluk halinde aktarım yapılabilir.
      </p>

      <h2 className={H2}>6. Verilerin Toplanma Yöntemleri</h2>
      <ul className={UL}>
        <li>Web sitesi kayıt formu, giriş, hesap güncelleme</li>
        <li>Ek-3 ve diğer modüllerde girilen mühendislik verileri (proje, yapı, taraflar)</li>
        <li>API entegrasyonları (ETABS Bridge — masaüstü uygulama, Iyzico)</li>
        <li>Çerezler (oturum, tercih, analitik — kullanım için açık rızanız gereklidir)</li>
      </ul>
      ...
      <ol className={OL}>
        ...
        <li>
          Düzeltme/silme işlemlerinin verilerinizi aktardığımız üçüncü kişilere bildirilmesini talep
          etme
        </li>
      ...
      <p className={P}>
        ...
        adresine yazılı başvurunuzu iletebilirsiniz. Başvurularınız 30 gün içinde yanıtlanır.
      </p>

      <h2 className={H2}>8. Saklama Süreleri</h2>
      <ul className={UL}>
        <li>Hesap aktif olduğu sürece ve hesap kapatıldıktan sonra 5 yıl (Vergi Usul Kanunu)</li>
        <li>
          Ek-3 belgeleri: yapı tamamlanıp iskan ruhsatı sonrası 30 yıl (yapı denetim mevzuatı)
        </li>
      ...
      <p className={P}>
        Bu metin güncellendiğinde önemli değişiklikler kayıtlı kullanıcılara e-posta ile bildirilir;
        sürekli erişilebilir olarak bu sayfada yayınlanır.
      </p>
    </article>
  );
}
